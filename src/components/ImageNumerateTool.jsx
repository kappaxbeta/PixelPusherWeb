...
const ImageNumerateTool = () => {
    ...
    const [startPos, setStartPos] = useState({ x: 0, y: 0, initialX: 50, initialY: 50 });
    ...
    const processSelection = () => {
        ...
        const maxRow = Math.min(rows - 1, Math.max(selectionStart.row, selectionEnd.row));
        const minCol = Math.max(0, Math.min(selectionStart.col, selectionEnd.col));
        const maxCol = Math.min(cols - 1, Math.max(selectionStart.col, selectionEnd.col));
    ...
    const renderGrid = () => {
        ...
        const minRow = selectionStart && selectionEnd ? Math.min(selectionStart.row, selectionEnd.row) : -1;
        const maxRow = selectionStart && selectionEnd ? Math.max(selectionStart.row, selectionEnd.row) : -1;
        const minCol = selectionStart && selectionEnd ? Math.min(selectionStart.col, selectionEnd.col) : -1;
        const maxCol = selectionStart && selectionEnd ? Math.max(selectionStart.col, selectionEnd.col) : -1;
            ...
            <div className="tool-layout">
                <aside className="tool-sidebar">
                    ...
                    <div className="info-panel">
                        ...
                        {totalTiles > 5000 && <p className="warning">⚠️ Only displaying first 5000 tiles</p>}
                    </div>
                </aside>
